...
import { BoldIcon, ItalicIcon, LucideIcon, UploadIcon, SearchIcon, ChevronDownIcon, HighlighterIcon, MessageSquarePlusIcon, RemoveFormattingIcon, ListTodoIcon, PrinterIcon, Redo2Icon, SpellCheck, UnderlineIcon, Undo2Icon, Link2Icon, ImageIcon, icons, AlignCenterIcon, AlignRightIcon, AlignLeftIcon, AlignJustifyIcon, ListIcon, ListOrderedIcon, MinusIcon, PlusIcon, ListCollapseIcon } from 'lucide-react';
...
const HeadingLevelButton = () => {
    ...
    return (
        <DropdownMenu>
            ...
            <DropdownMenuContent className='p-1 flex flex-col gap-py-1'>
                {headings.map(({ label, value, fontSize }) => (
                    ...
                    }} key={value} style={{ fontSize }} className={cn("flex items-center gap-x-2 py-1 px-2 rounded-sm hover:bg-neutral-200/80", (value === 0 && !editor?.isActive("heading")) || editor?.isActive("heading", { level: value }) && "bg-neutral-200/80")}>
                        {label}
                    </button>
                ))}
            </DropdownMenuContent>
        ...
        <DropdownMenu>
            ...
            <DropdownMenuContent className='p-1 flex flex-col gap-y-1'>
                {fonts.map(({ label, value }) => (
                    <button onClick={() => editor?.chain().focus().setFontFamily(value).run()} key={value} className={cn("flex items-center gap-x-2 py-1 px-2 rounded-sm hover:bg-neutral-200/80", editor?.getAttributes("textStyle").fontFamily === value && "bg-neutral-200/80")} style={{ fontFamily: value }}>
